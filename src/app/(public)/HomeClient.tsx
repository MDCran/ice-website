...
export interface HomePageData {
  services_grid?: { items: { icon: string; title: string; description: string; href: string }[] };
  stats?: { items: { value: number; suffix: string; label: string }[] };
  timeline?: { items: { year: string; title: string; description: string }[] };
  industries_cta?: { items: { name: string; icon: string }[] };
  partners_marquee?: { partners: string[] };
  hero?: { headline: string; subheadline: string; cta_primary?: { label: string; href: string }; cta_secondary?: { label: string; href: string } };
  data_centers?: { heading: string; description: string; features: string[] };
...
const GRADIENTS = [
  { gradient: "from-sky-500/20 to-blue-500/20", glowColor: "group-hover:shadow-sky-500/20" },
  { gradient: "from-blue-500/20 to-sky-500/20", glowColor: "group-hover:shadow-blue-500/20" },
  { gradient: "from-blue-500/20 to-sky-500/20", glowColor: "group-hover:shadow-blue-500/20" },
  { gradient: "from-sky-500/20 to-blue-500/20", glowColor: "group-hover:shadow-sky-500/20" },
];
...
export default function Home({ data }: { data?: HomePageData }) {
  ...
  const services = (data?.services_grid?.items ?? DEFAULT_SERVICES).map((s, i) => ({
    icon: resolveIcon(s.icon),
    title: s.title,
    description: s.description,
    href: s.href,
    gradient: GRADIENTS[i % GRADIENTS.length].gradient,
    glowColor: GRADIENTS[i % GRADIENTS.length].glowColor,
  }));
  const stats: { value: number; suffix: string; label: string; icon: LucideIcon }[] =
    (data?.stats?.items ?? DEFAULT_STATS).map((s, i) => ({
      ...s,
        ...
        <div className="relative mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            ...
            <div className="mt-10 flex flex-wrap justify-center gap-4">
              ...
              <a href="tel:18007869188" className="btn-outline">
