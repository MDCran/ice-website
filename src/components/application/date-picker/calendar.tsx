...
export const CalendarContextProvider = ({ children }: PropsWithChildren) => {
    ...
    // react-aria-components >=1.19 supports multi-date selection, so `onChange`
    // may receive an array; unwrap it to keep single-date state.
    const handleChange = (next: DateValue | readonly DateValue[]) => onChange(Array.isArray(next) ? ((next[0] as DateValue | undefined) ?? null) : (next as DateValue));
