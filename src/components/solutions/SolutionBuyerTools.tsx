...
export function SolutionArchitecture({ slug }: { slug: string }) {
  ...
  const architecture = data.architecture.length > 0 ? data.architecture : ["Source", "Secure edge", "Managed platform", "Protected outcome"];
  const firstLayer = architecture[0] ?? "Source";
  const finalLayer = architecture[architecture.length - 1] ?? "Managed outcome";
    ...
    <section className="border-b border-secondary bg-primary py-16 md:py-24">
      <div className="mx-auto max-w-container px-4 md:px-8">
        ...
        <div className="ice-arch-live-surface relative mx-auto mt-12 max-w-6xl overflow-hidden rounded-3xl border border-white/10 bg-[#07152a] p-5 shadow-[0_24px_80px_rgb(2_12_27/0.26)] md:p-8">
          ...
          {!reduceMotion && <div aria-hidden="true" className="ice-arch-ambient-flow pointer-events-none absolute inset-0" />}
          {!reduceMotion && <div aria-hidden="true" className="ice-arch-scanner pointer-events-none absolute inset-y-0 left-0 w-1/3" />}
          ...
          <div className="relative flex flex-col gap-3 border-b border-white/10 pb-5 sm:flex-row sm:items-center sm:justify-between">
            ...
            <span className="inline-flex w-fit items-center gap-3 rounded-full bg-emerald-400/10 px-3 py-1.5 text-xs font-semibold text-emerald-300 ring-1 ring-emerald-300/20">
              ...
              <span className="tabular-nums">
                {String(architecture.length).padStart(2, "0")} live layers
              </span>
            </span>
          </div>
          ...
          <div className="relative mt-5 overflow-hidden rounded-full bg-white/[0.06] p-1 ring-1 ring-white/10">
            <span
              className={cx("block h-1.5 w-full rounded-full bg-gradient-to-r from-brand-500 via-sky-200 to-emerald-300 shadow-[0_0_20px_rgb(4_155_251/0.55)]", !reduceMotion && "ice-arch-progress-stream ice-arch-flow-fill")}
              aria-hidden="true"
            />
            {!reduceMotion && <>
              <span aria-hidden="true" className="ice-arch-pipeline-sheen absolute inset-y-0 left-0 w-1/3" />
              <span aria-hidden="true" className="ice-arch-pipeline-sheen ice-arch-pipeline-sheen-delayed absolute inset-y-0 left-0 w-1/4" />
              <span aria-hidden="true" className="ice-arch-data-packet absolute top-1/2 left-0" />
              <span aria-hidden="true" className="ice-arch-data-packet ice-arch-data-packet-delayed absolute top-1/2 left-0" />
            </>}
          </div>

          <div className="relative mt-8 hidden md:block">
            <div aria-hidden="true" className={cx("absolute top-8 right-[9%] left-[9%] h-[3px] overflow-visible rounded-full bg-white/15", !reduceMotion && "ice-arch-rail-base")}>
              <span
                className={cx("absolute inset-y-0 left-0 w-full rounded-full bg-gradient-to-r from-brand-500 via-sky-200 to-emerald-300 shadow-[0_0_18px_rgb(4_155_251/0.7)]", !reduceMotion && "ice-arch-progress-stream ice-arch-flow-fill")}
                aria-hidden="true"
              />
              {!reduceMotion && <>
                ...
                <span aria-hidden="true" className="ice-arch-data-packet absolute top-1/2 left-0" />
                <span aria-hidden="true" className="ice-arch-data-packet ice-arch-data-packet-delayed absolute top-1/2 left-0" />
                <span aria-hidden="true" className="ice-arch-data-packet ice-arch-data-packet-fast absolute top-1/2 left-0" />
              </>}
            </div>
                ...
                return (
                  ...
                  >
                    <span
                      ...
                      className={cx("relative flex size-16 items-center justify-center rounded-2xl border border-brand-300/40 bg-brand-500/20 text-sm font-bold text-white shadow-[0_0_18px_rgb(4_155_251/0.22)]", !reduceMotion && "ice-arch-node-live")}
                    >
                      {!reduceMotion && (
                        <span
                    ...
                    <span className="mt-4 max-w-36 text-sm font-semibold leading-snug text-white/85 transition-colors duration-500">
                      {step}
                    </span>
                    <span className="mt-2 text-[10px] font-semibold tracking-[0.16em] text-brand-300 uppercase">
                      Live
                    </span>
                    <span className="mt-2 h-1 w-10 overflow-hidden rounded-full bg-white/10 opacity-100">
                      <span style={{ animationDelay: `${index * 0.22}s` }} className={cx("block h-full rounded-full bg-brand-300", !reduceMotion && "ice-arch-node-meter")} />
                    </span>
                  </li>
                );
              })}
          ...
          <ol className="relative mt-6 grid gap-3 md:hidden">
            <div aria-hidden="true" className={cx("absolute top-5 bottom-5 left-5 w-px bg-white/15", !reduceMotion && "ice-arch-vertical-flow")} />
            {architecture.map((step, index) => {
              const Icon = ARCHITECTURE_NODE_ICONS[index % ARCHITECTURE_NODE_ICONS.length];
              return (
                <li key={step} className="relative flex items-center gap-4 rounded-xl border border-brand-300/25 bg-brand-500/10 p-3 transition-colors duration-500">
                  <span style={{ animationDelay: `${index * -0.22}s` }} className={cx("relative z-10 flex size-10 shrink-0 items-center justify-center rounded-xl bg-brand-500/25 text-white shadow-[0_0_16px_rgb(4_155_251/0.22)]", !reduceMotion && "ice-arch-node-live")}>
                    {!reduceMotion && (
                      <span
          ...
          <div
            className={cx("relative mt-8 grid gap-4 rounded-2xl border border-brand-300/20 bg-brand-500/10 p-4 ring-1 ring-white/[0.04] md:grid-cols-[0.8fr_1.2fr] md:p-5", !reduceMotion && "ice-arch-layer-card")}
          >
            <div>
              <p className="text-[11px] font-semibold tracking-[0.18em] text-brand-200 uppercase">Live service path</p>
              <h3 className="mt-2 text-xl font-semibold text-white">{firstLayer} to {finalLayer}</h3>
            </div>
